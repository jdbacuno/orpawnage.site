<x-admin-layout>
  ...
  <div class="mt-4">
      ...
      <div class="flex flex-wrap gap-2">
        <form method="GET" action="{{ request()->url() }}#pets" class="flex flex-wrap gap-4">
          ...
          <select name="reproductive_status"
            ...
            <option value="neutered" {{ request('reproductive_status')=='neutered' ? 'selected' : '' }}>Neutered/Spayed</option>
          </select>
          ...
          <select name="adoption_status"
            ...
            <option value="in_process" {{ request('adoption_status')=='in_process' ? 'selected' : '' }}>In Process</option>
          </select>
          ...
          <select name="sort_by"
            ...
            <option value="youngest" {{ request('sort_by')=='youngest' ? 'selected' : '' }}>Youngest First</option>
            <option value="oldest_age" {{ request('sort_by')=='oldest_age' ? 'selected' : '' }}>Oldest First</option>
          </select>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      @foreach($pets as $pet)
      <div
        class="relative bg-white w-full mx-auto rounded-xl shadow-sm overflow-hidden transition-all duration-200 border border-gray-200 pet-card cursor-pointer"
        wire:poll.10s data-pet-id="{{ $pet->id }}">
        <img src="{{ asset('storage/' . ($pet->image_path ?? 'pet-images/catdog.svg')) }}" alt="Pet Image"
          class="h-64 w-full object-cover transition-all duration-300" />
        ...
        <div
          class="absolute bottom-0 left-0 w-full bg-white/70 backdrop-blur-md text-gray-900 p-4 translate-y-full transition-all duration-300 ease-in-out slide-panel">
          <div class="flex justify-between items-start mb-3">
            ...
            <div class="relative">
              <button
                ...
                data-menu-id="menu-{{ $pet->id }}" onclick="event.stopPropagation();">
                <i class="ph-fill ph-dots-three-outline-vertical text-lg text-gray-700"></i>
              </button>
              <div id="menu-{{ $pet->id }}"
                class="hidden absolute right-0 top-8 w-44 bg-white rounded-md shadow-lg z-10 border border-gray-100">
                <div class="py-1">
                  ...
                  <button
                    ...
                    data-species="{{ $pet->species }}" data-breed="{{ $pet->breed ?? '' }}" data-age="{{ $pet->age }}" data-age-unit="{{ $pet->age_unit }}"
          ...
          <div class="flex flex-wrap gap-2 mb-2">
            <span class="bg-blue-100 text-blue-800 text-xs px-3 py-1 rounded-full border border-blue-200 cursor-pointer"
              ...
              {{ $pet->formatted_age }} {{ $pet->age == 1 ? Str::singular($pet->age_unit) : Str::plural($pet->age_unit) }} old
            </span>
            <span
            ...
            @if($pet->breed)
            <span
              class="bg-indigo-100 text-indigo-800 text-xs px-3 py-1 rounded-full border border-indigo-200 cursor-pointer"
              data-description="Breed" onclick="changeText(this)">
              {{ ucfirst($pet->breed) }}
            </span>
            @endif
            <span
            ...
            @endif
            <span class="bg-gray-100 text-gray-800 text-xs px-3 py-1 rounded-full border border-gray-200 cursor-pointer"
              data-description="Source" onclick="changeText(this)">
              {{ ucfirst($pet->source) }}
            </span>
            <span
              class="{{ $pet->reproductive_status == 'neutered' ? 'bg-green-50 text-green-800 border-green-100' : 'bg-amber-50 text-amber-800 border-amber-100' }} text-xs px-3 py-1 rounded-full border cursor-pointer"
  ...
  <!-- Add Pet Modal -->
  <div id="modal" class="fixed inset-0 px-1 flex items-center justify-center bg-black bg-opacity-50 z-50 {{ session('modal_open') === 'add' ? '' : 'hidden' }}">
    <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-2xl max-h-[70vh] overflow-hidden relative flex flex-col">
      ...
      <div class="flex flex-col overflow-y-auto scrollbar-hidden p-2 space-y-4 flex-grow">
        ...
        <div class="flex justify-center relative">
          ...
          <img id="imagePreview" src="" class="w-full h-auto object-cover rounded-lg border border-gray-300 hidden" alt="Pet Image">
        </div>

        <form method="POST" id="addPetForm" action="/admin/pet-profiles" enctype="multipart/form-data" class="space-y-6">
          @csrf

          <div>
            <label class="block text-sm font-medium text-gray-600 mb-1">Pet Image <span class="text-red-500">*</span></label>
            <input type="file" name="image" id="imageInput"
              class="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-orange-50 file:text-orange-700 hover:file:bg-orange-100" required>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-4">
              <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">Pet Number <span class="text-red-500">*</span></label>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">Pet Name <span class="text-red-500">*</span></label>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">Species <span class="text-red-500">*</span></label>
                <select name="species"
              ...
              <div>
                ...
                <input type="text" name="breed" placeholder="e.g., Labrador, Persian (Optional)"
                  class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400"
                  value="{{ old('breed') }}">
            ...
            <div class="space-y-4">
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-sm font-medium text-gray-600 mb-1">Age <span class="text-red-500">*</span></label>
                  <input type="number" name="age" placeholder="Age" step="0.1" min="0.1" max="100"
                    class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400"
              ...
              <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">Sex <span class="text-red-500">*</span></label>
                <select name="sex"
                  class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400" required>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">Reproductive Status <span class="text-red-500">*</span></label>
                <select name="reproductive_status"
                  class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400" required>
                  <option value="intact" {{ old('reproductive_status')==='intact' ? 'selected' : '' }}>Intact</option>
                  <option value="neutered" {{ old('reproductive_status')==='neutered' ? 'selected' : '' }}>Neutered/Spayed</option>
                </select>
                <x-form-error name="reproductive_status" />
              </div>

              <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">Color <span class="text-red-500">*</span></label>
                <select name="color"
                  class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400" required>
                  @foreach (["black", "white", "gray", "brown", "orange", "brindle", "calico", "tabby", "bi-color", "tri-color", "others"] as $color)
              ...
              <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">Source <span class="text-red-500">*</span></label>
                <select name="source"
                  class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400" required>
  ...
  <!-- Edit Pet Modal -->
  <div id="editModal" class="fixed inset-0 px-1 flex items-center justify-center bg-black bg-opacity-50 z-50 {{ session('modal_open') === 'edit' ? '' : 'hidden' }}">
    <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-2xl max-h-[70vh] overflow-hidden relative flex flex-col">
      ...
      <div class="flex flex-col overflow-y-auto scrollbar-hidden p-2 space-y-4 flex-grow">
        ...
        @php
        $editPetData = session('edit_pet_data');
        @endphp

        <div class="flex justify-center relative">
          <p id="editImagePlaceholder" class="absolute text-gray-500 text-sm">Current pet image</p>
          <img id="editImagePreview"
            src="{{ $editPetData && $editPetData->image_path ? asset('storage/' . $editPetData->image_path) : '' }}"
            class="w-full h-auto object-cover rounded-lg border border-gray-300 {{ $editPetData && $editPetData->image_path ? '' : 'hidden' }}" alt="Pet Image">
        </div>

        <form method="POST" id="editPetForm"
          action="{{ $editPetData ? '/admin/pet-profiles/' . $editPetData->id : '#' }}" enctype="multipart/form-data" class="space-y-6">
          @csrf
          @method('PATCH')

          <input type="hidden" name="id" id="editPetId" value="{{ $editPetData ? $editPetData->id : '' }}">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-4">
              <div>
                ...
                <input type="number" name="pet_number" id="editPetNumber"
                  class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400"
                  value="{{ old('pet_number', $editPetData ? $editPetData->pet_number : '') }}" min="1" max="100" required>
                <x-form-error name="pet_number" />
              </div>
              ...
              <div>
                ...
                <input type="text" name="pet_name" id="editPetName"
                  class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400"
                  placeholder="(Optional)" value="{{ old('pet_name', $editPetData ? $editPetData->pet_name : '') }}">
                <x-form-error name="pet_name" />
              </div>
              ...
              <div>
                ...
                <select name="species" id="editSpecies"
                  class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400" required>
                  <option value="feline" {{ old('species', $editPetData ? $editPetData->species : '') == 'feline' ? 'selected' : '' }}>Feline (Cat)</option>
                  <option value="canine" {{ old('species', $editPetData ? $editPetData->species : '') == 'canine' ? 'selected' : '' }}>Canine (Dog)</option>
                </select>
                <x-form-error name="species" />
              </div>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-600 mb-1">Breed</label>
                <input type="text" name="breed" id="editBreed" placeholder="e.g., Labrador, Persian (Optional)"
                  class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400"
                  value="{{ old('breed', $editPetData ? $editPetData->breed : '') }}">
                <x-form-error name="breed" />
              </div>
            </div>
            ...
            <div class="space-y-4">
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-sm font-medium text-gray-600 mb-1">Age <span class="text-red-500">*</span></label>
                  <input type="number" name="age" id="editAge" step="0.1" min="0.1" max="100"
                    class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400"
                    value="{{ old('age', $editPetData ? $editPetData->age : '') }}" required>
                  <x-form-error name="age" />
                </div>
                <div>
                  <label class="block text-sm font-medium text-gray-600 mb-1">Unit <span class="text-red-500">*</span></label>
                  <select name="age_unit" id="editAgeUnit"
                    class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400" required>
                    <option value="months" {{ old('age_unit', $editPetData ? $editPetData->age_unit : '') == 'months' ? 'selected' : '' }}>Months</option>
                    <option value="years" {{ old('age_unit', $editPetData ? $editPetData->age_unit : '') == 'years' ? 'selected' : '' }}>Years</option>
                    <option value="weeks" {{ old('age_unit', $editPetData ? $editPetData->age_unit : '') == 'weeks' ? 'selected' : '' }}>Weeks</option>
                  </select>
                  <x-form-error name="age_unit" />
                </div>
              </div>
              ...
              <div>
                ...
                <select name="sex" id="editSex"
                  class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400" required>
                  <option value="male" {{ old('sex', $editPetData ? $editPetData->sex : '') == 'male' ? 'selected' : '' }}>Male</option>
                  <option value="female" {{ old('sex', $editPetData ? $editPetData->sex : '') == 'female' ? 'selected' : '' }}>Female</option>
                </select>
                <x-form-error name="sex" />
              </div>
              ...
              <div>
                ...
                <select name="reproductive_status" id="editReproStatus"
                  class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400" required>
                  <option value="intact" {{ old('reproductive_status', $editPetData ? $editPetData->reproductive_status : '') == 'intact' ? 'selected' : '' }}>Intact</option>
                  <option value="neutered" {{ old('reproductive_status', $editPetData ? $editPetData->reproductive_status : '') == 'neutered' ? 'selected' : '' }}>Neutered/Spayed</option>
                </select>
                <x-form-error name="reproductive_status" />
              </div>
              ...
              <div>
                ...
                <select name="color" id="editColor"
                  class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400" required>
                  @foreach (["black", "white", "gray", "brown", "orange", "brindle", "calico", "tabby", "bi-color", "tri-color", "others"] as $color)
                  <option value="{{ $color }}" {{ old('color', $editPetData ? $editPetData->color : '') == $color ? 'selected' : '' }}>{{ ucfirst($color) }}</option>
                  @endforeach
                </select>
                <x-form-error name="color" />
              </div>
              ...
              <div>
                ...
                <select name="source" id="editSource"
                  class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400" required>
                  <option value="surrendered" {{ old('source', $editPetData ? $editPetData->source : '') == 'surrendered' ? 'selected' : '' }}>Surrendered</option>
                  <option value="rescued" {{ old('source', $editPetData ? $editPetData->source : '') == 'rescued' ? 'selected' : '' }}>Rescued</option>
                </select>
                <x-form-error name="source" />
              </div>
            </div>
  ...
  <!-- Archive Modal -->
  <div id="archiveModal" class="fixed inset-0 px-1 flex items-center justify-center bg-black bg-opacity-50 z-50 {{ $errors->has('archive_reason') || $errors->has('archive_notes') ? '' : 'hidden' }}">
    <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-md relative">
        ...
        <div class="flex items-center">
          ...
          <div class="ml-3">
            <p class="text-sm text-yellow-700">
              You are about to archive Pet#<span id="archivePetIdText" class="font-semibold">{{ old('pet_number', session('archive_pet_number')) }}</span>.
      ...
      <form id="archivePetForm" method="POST" action="{{ old('pet_id') ? url('/admin/pet-profiles/' . old('pet_id') . '/archive') : '' }}" class="mt-6">
        @csrf
        @method('PATCH')
        <input type="hidden" name="pet_id" id="archivePetId" value="{{ old('pet_id') }}">

        <div class="mb-4">
          <label class="block text-sm font-medium text-gray-600 mb-1">Reason for Archiving <span class="text-red-500">*</span></label>
          <select name="archive_reason" id="archiveReason"
            class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-2 focus:ring-orange-300 focus:border-orange-400" required>
            <option value="">Select a reason</option>
            <option value="Pet has passed away" {{ old('archive_reason')=='Pet has passed away' ? 'selected' : '' }}>Pet has passed away</option>
            <option value="Pet has health issues" {{ old('archive_reason')=='Pet has health issues' ? 'selected' : '' }}>Pet has health issues</option>
            <option value="Other" {{ old('archive_reason')=='Other' ? 'selected' : '' }}>Other (please specify)</option>
          </select>
          <x-form-error name="archive_reason" />
        </div>

        <div class="mb-4" id="archiveNotesContainer" style="{{ old('archive_reason') == 'Other' ? '' : 'display: none;' }}">
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      petCards.forEach(card => {
        ...
        if (image) {
          image.addEventListener('click', function(e) {
            e.stopPropagation();
            const slidePanel = card.querySelector('.slide-panel');
            const isActive = slidePanel.classList.contains('translate-y-0');

            petCards.forEach(otherCard => {
              if (otherCard !== card) {
                const otherPanel = otherCard.querySelector('.slide-panel');
                otherPanel.classList.remove('translate-y-0');
                otherPanel.classList.add('translate-y-full');
                otherCard.classList.remove('shadow-md');
              }
            });

            if (isActive) {
              slidePanel.classList.remove('translate-y-0');
              slidePanel.classList.add('translate-y-full');
              card.classList.remove('shadow-md');
            } else {
              slidePanel.classList.remove('translate-y-full');
              slidePanel.classList.add('translate-y-0');
      ...
      petCards.forEach(card => {
        const slidePanel = card.querySelector('.slide-panel');
        if (slidePanel) {
          slidePanel.addEventListener('click', function(e) {
            e.stopPropagation();
          });
        }
      });
      ...
      document.addEventListener('click', function(e) {
        if (!e.target.closest('.menu-toggle') && !e.target.closest('[id^="menu-"]')) {
          menuToggles.forEach(toggle => {
      ...
      document.querySelectorAll('.edit-btn').forEach(button => {
        button.addEventListener('click', function() {
          ...
          document.getElementById('editPetNumber').value = petNumber;
          ...
          document.getElementById('editAgeUnit').value = ageUnit;
          document.getElementById('editSex').value = sex;
          document.getElementById('editReproStatus').value = reproStatus;
