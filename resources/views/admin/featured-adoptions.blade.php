<x-admin-layout>
  ...
  <div id="alert-3" class="flex items-center p-4 mb-4 text-green-800 rounded-lg bg-green-50" role="alert">
    <svg class="flex-shrink-0 w-4 h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
      viewBox="0 0 20 20">
      <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0
1 0 2Z" />
    </svg>
  ...
  <div class="py-6">
    ...
    <div class="px-6 py-3 mb-8 bg-gray-50 rounded-lg border border-gray-200">
	  ...
	  <p class="text-xs text-gray-500 mt-1">The reconmmended size of each file is 1 MB or below.</p>
	  <p class="text-xs text-gray-500 mt-1">You can upload 5 photos at a time and edit their adoption date after uploading.</p>
      ...
      <div class="month-section" id="{{ Str::slug($month) }}" data-month="{{ date('n', strtotime($month)) }}"
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 gallery-grid">
          ...
          <div
            ...
            data-year="{{ date('Y', strtotime($month)) }}" data-id="{{ $image->id }}">
            <div class="relative aspect-square"
              ...
              <div class="absolute top-2 right-2 flex flex-col gap-1">
                <button type="button"
                  class="bg-white/90 hover:bg-white text-blue-600 hover:text-blue-700 rounded-full p-1 shadow hidden group-hover:block"
                  title="Edit Date"
                  onclick="event.stopPropagation(); openEditDateModal('{{ $image->id }}', '{{ $image->created_at->format('Y-m-d\TH:i') }}')">
                ...
                <button type="button"
                  class="bg-white/90 hover:bg-white text-red-600 hover:text-red-700 rounded-full p-1 shadow hidden group-hover:block"
                  title="Delete"
                  onclick="event.stopPropagation(); confirmDelete('{{ route('featured-adoptions.destroy', $image) }}')">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd"
                      d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2h12a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zm-3 6a1 1 0 011 1v6a1 1 0 11-2 0V9a1 1 0 011-1zm4 0a1 1 0 011 1v6a1 1 0 11-2 0V9a1 1 0 011-1zm5 1a1 1 0 10-2 0v6a1 1 0 102 0V9z"
                      clip-rule="evenodd" />
                  </svg>
                </button>
              </div>
  ...
  <div id="editDateModal"
    ...
    <div class="relative mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
      <div class="mt-3 text-center">
        ...
        <h3 class="text-lg leading-6 font-medium text-gray-900 mt-2">Edit Adoption Date</h3>
        <div class="mt-2 px-7 py-3">
          <form id="editDateForm" method="POST">
            ...
            <div class="text-left">
              <label for="edit_created_at" class="block text-sm font-medium text-gray-700 mb-1">Adoption Date</label>
            ...
            <div class="items-center px-4 py-3 mt-4">
              <button type="button" id="cancelEditDate"
                class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 mr-2">
                Cancel
              </button>
  ...
  <div class="max-w-7xl w-full max-h-[70vh] flex items-center justify-center" onclick="closeImageModal()">
    <!-- Image container with relative positioning -->
    <div class="relative inline-block" onclick="event.stopPropagation()">
      <img id="modalImage" src="" alt="Preview" class="max-h-[70vh] max-w-full rounded-lg shadow-lg">
      <!-- Close button positioned relative to the image -->
      <button type="button" class="absolute top-2 right-2 z-10 text-white bg-black/50 hover:bg-black/70 rounded-full p-2 transition-colors"
        aria-label="Close preview" onclick="event.stopPropagation(); closeImageModal()">
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      setupImageUpload('images_input', 'imagesPreviews', 'addImagesBtn', 5);
    ...
    // Edit Date Modal
    function openEditDateModal(imageId, currentDate) {
      const modal = document.getElementById('editDateModal');
      const form = document.getElementById('editDateForm');
      const imageIdInput = document.getElementById('edit_image_id');
      const dateInput = document.getElementById('edit_created_at');
      const cancelBtn = document.getElementById('cancelEditDate');
    ...
    document.getElementById('editDateForm').addEventListener('submit', function(e) {
      ...
      .then(data => {
        if (data.success) {
          ...
          // Reload the page to see the updated grouping
