    ...
    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
        ...
        <div>
            <span class="relative z-0 inline-flex rtl:flex-row-reverse shadow-sm rounded-md">
                ...
                if ($current <= 2) $end=min(5, $last); if ($current>= $last - 1) $start = max($last - 4, 1);
                    @endphp

                    @if ($start > 1)
                    <button wire:click="gotoPage(1)"
                        class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700 bg-white border border-gray-300 leading-5 hover:text-black hover:bg-yellow-400 transition ease-in-out duration-150">
                        1
                    </button>
                    @if ($start > 2)
                    <span
                    ...
                    @for ($page = $start; $page <= $end; $page++) @if ($page==$paginator->currentPage())
                        <span aria-current="page">
                            <span
                                class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-black bg-yellow-400 border border-gray-300 cursor-default leading-5">
                                {{ $page }}
                            </span>
                        </span>
                        @else
